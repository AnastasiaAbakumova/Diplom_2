...
import pytest
import allure
from conftest import BASE_URL, unique_user, registered_user
...
class TestUserRegistration:
    ...
    def test_create_unique_user(self, unique_user):
        with allure.step("Формируем данные уникального пользователя"):
            payload = {
    ...
    def test_create_user_missing_field(self, payload, missing_field):
        # Здесь убрали пустой step
        with allure.step("Отправляем POST-запрос на регистрацию без обязательного поля"):
            response = requests.post(f"{BASE_URL}/auth/register", json=payload)
